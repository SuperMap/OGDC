...
public:
	...
	//! \brief 得到类型, 固定为 GeoCompoundParamLine
...
//protected: //! 子类需要实现的一些不公开函数
	...
	//! \brief 在最后增加一个子对象,此对象只能是UGGeoParametricLine对象。
	//! \return pSub非法, 返回false。
	  UGbool AddSub( UGGeometry *pGeometry);	

	//! \brief 插入子对象。
	//! \param nIndex [in] 要插入的位置索引。
	//! \param pGeoLine [in] 子对象指针,此对象只能是UGGeoParametricLine对象。
	//! \return nSubIndex或者pGeoLine非法,返回false。
	  UGbool InsertSub(UGint nSubIndex, UGGeometry *pGeometry);

	//! \brief 给指定的位置设置结合对象。
	//! \param nIndex [in] 指定的位置索引。
	//! \param pGeometry [in] 几何对象指针,此对象只能是UGGeoParametricLine对象。
	...
	//! \return 返回子对象UGGeoParametricLine指针，索引非法, 返回false。
