...
public:
	...
	//! \brief 获取金字塔数据集指针
	virtual OgdcDatasetRaster * GetPyramidTier(OgdcInt nImgWidth,OgdcInt nDeviceWidth);
	...
	virtual OgdcLong GetWidth() const;	
	
	//! \brief 获取栅格数据集高度，以象素值为单位
	//! \return 栅格数据集高度，以象素值为单位
	virtual OgdcLong GetHeight() const;	
    ...
    //! \param nBandIndex[in] 获取颜色数值所在的波段索引值，默认为0
	//! \return 是否获取成功。
	virtual OgdcBool GetScanline(OgdcInt nRowIndex,OgdcRasterScanline& scanLine, OgdcInt nBandIndex=0,OgdcInt nStartCol = 0);
	...
	//! \breif 保存所有块数据
	virtual void SaveAllBlocks(OgdcInt nBandIndex=0);
